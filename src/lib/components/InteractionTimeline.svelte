...
{:else}
	<ol class="flex flex-col gap-2" data-testid="interaction-timeline">
		{#each items as item (item.id)}
			...
			<li class="flex gap-3 rounded-app bg-card px-4 py-3 shadow-card">
				<span
					class="mt-0.5 flex size-7 shrink-0 items-center justify-center rounded-full"
					style="background:color-mix(in srgb, {kind.accent} 16%, transparent); color:{kind.accent}"
				>
					<Icon name={kind.icon} size={15} />
				</span>
				<div class="min-w-0 flex-1">
					<div class="flex flex-wrap items-baseline gap-x-2 gap-y-0.5">
						<span class="text-xs font-semibold" style="color:{kind.accent}">{kind.label}</span>
				...
				{#if item.mine}
					<form method="POST" action="?/removeInteraction" class="shrink-0">
						...
						<Button variant="danger" size="sm" icon="remove" label="Remove interaction" title="Remove" />
					</form>
				{/if}
			</li>
		{/each}
