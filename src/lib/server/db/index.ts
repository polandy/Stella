...
/**
 * The raw `bun:sqlite` handle behind the Drizzle one. Needed where a statement is assembled
 * from a table list rather than written against the typed schema — the export archive reads
 * every table that way (`archive-repository.ts`). Prefer `getDb()` everywhere else.
 */
export function getSqlite(): import('bun:sqlite').Database {
	getDb();
